# Squad Foxtrot

Sorted by first name. Fun facts are load-bearing at this company.

## Team

- **Ben Tran** — Data Analyst — fun fact: kept the retention autopsy.
- **Marcus Chen** — Engineering Manager — fun fact: reads everything, interrupts rarely.
- **Nadia Okafor** — Senior Engineer — fun fact: watches the repo feed like sports.
- **Sam** — Full-stack Engineer (departed) — fun fact: hired away — we're proud and furious.
- **Sofia Reyes** — Product Manager — fun fact: has never typed a semicolon.
- **Theo Lindqvist** — Product Designer — fun fact: lowercase, always.
- **_Your name here_** — Software Engineer I — fun fact: _replace this line — it is holding your spot._
